...
class GroupDetailsViewModel @Inject constructor(
    ...
    fun getGroupDetails(groupId: Long) {
        getGroupDetailsUseCase(groupId).onEach { result ->
            when (result) {
                is Resource.Success -> {
                    _getGroupDetailsState.value = GetGroupDetailsState(group = result.data)
                }
                is Resource.Error -> {
                    _getGroupDetailsState.value = GetGroupDetailsState(error = result.message ?: "Unknown error")
                }
                is Resource.Loading -> {
                    _getGroupDetailsState.value = GetGroupDetailsState(isLoading = true)
                }
            }
